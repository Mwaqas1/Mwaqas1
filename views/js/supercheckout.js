...
function addNewCustomerProfilePopup()
{
    ...
    $("#modal_add_new_customer_profile_form select option:selected").prop("selected", false);
    $("#modal_add_new_customer_profile_form select option:first").prop("selected", "selected");
    $('#modal_add_new_customer_profile_form').modal({'show': true, 'backdrop': 'static'});
}
...
//    $('#modal_edit_custom_field_form').html("");
    ...
    $.ajax( {
        type: "POST",
        url: scp_ajax_action,
        data: $('#supercheckout_configuration_form').serialize()+'&ajax=true&customer_profile_action=displayEditCustomerProfileForm&id_profile=' + idProfile,
        async: false,
        dataType: 'json',
        success: function(json) {
            $('#modal_edit_customer_profile_form').html(json.response);
            $("#modal_edit_customer_profile_form input[type='checkbox']").uniform(); 
            $('#modal_edit_customer_profile_form').modal({'show': true, 'backdrop': 'static'});
        }
    });
}
...
function submitCustomerProfileForm()
{
    ...
    else
    {
        ...
        $.ajax( {
            type: "POST",
            url: scp_ajax_action,
            data: $('#supercheckout_configuration_form').serialize()+'&ajax=true&customer_profile_action=addCustomerProfileForm',
            async: true,
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function(json) {
                ...
                var tableRow = '<tr class="pure-table-striped row_changed" id="tr_pure_table_'+json.id_profile+'">';
                tableRow += '<td class="center">' + rowCount + '</td>';
                tableRow += '<td class="width_25 center"><div class="div_250px_ellipsis">' + json.field_label + '</div></td>';
                tableRow += '<td class="center">' + activeText + '</td>';
                tableRow += '<td class="center" style="padding: 12px;">';
                tableRow += '<a style="margin-top: -26px;" href="javascript://" onclick="displayEditCustomerProfilePopup(' + json.id_profile + ')" type="11" class="velsof-glyphicons2 glyphicons pencil"><i data-toggle="tooltip" data-placement="top" data-original-title="Edit this custom field"></i></a>';
                tableRow += '<a style="margin-top: -26px;" href="javascript://" onclick="deleteCustomerProfileRow(' + json.id_profile + ')" type="11" class="velsof-glyphicons2 glyphicons bin" onclick=""><i data-toggle="tooltip" data-placement="top" data-original-title="Delete this custom field."></i></a>';
                tableRow += '</td>';
                tableRow += '</tr>';

                // Removing the success green color from all the previous edited/added rows
                $("#table_customer_profile_data").children().removeClass("row_changed");
                $("#tr_customer_profile_add_new").before(tableRow);
                $("#div_custom_fields_success").removeClass("hidden_custom");
                $("#loader_add_form_customer_profile").addClass("hidden_custom");
                setTimeout(function(){
                    $("#table_customer_profile_data").children().removeClass("row_changed", 1000);
                }, 5000);
                }
            }
        });
...
{ 
    ...
    else
    {
        ...
        $.ajax( {
            type: "POST",
            url: scp_ajax_action,
            data: $('#supercheckout_configuration_form').serialize()+'&ajax=true&customer_profile_action=editCustomerProfileForm',
            async: true,
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function(json) {
                ...
                var findRowCount = $("#tr_pure_table_"+json.id_profile).children("td:first").text();
                var tableRow = '<tr class="row_changed" id="tr_pure_table_'+json.id_profile+'">';
                ...
                // Removing the success green color from all the previous edited/added rows
                $("#tbody_customer_profile_data").children().removeClass("row_changed");
                $("#tr_pure_table_"+json.id_profile).replaceWith(tableRow);
                $("#div_custom_fields_success").removeClass("hidden_custom");
                $("#loader_edit_form_customer_profile").addClass("hidden_custom");
                setTimeout(function(){
                    $("#tbody_customer_profile_data").children().removeClass("row_changed", 1000);
                }, 5000); 
            }
        } 
        });
...
function deleteCustomerProfileRow(idCustomerProfile)
{
    ...
    if(canDelete == true)
    {
        ...
        $.ajax( {
            type: "POST",
            url: scp_ajax_action,
            data: '&ajax=true&customer_profile_action=deleteCustomerProfileRow&id_profile='+idCustomerProfile,
            async: false,
            dataType: 'json',
            success: function(json) {
